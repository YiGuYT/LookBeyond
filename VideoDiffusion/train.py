...
def main():
    ...
    for name, para in channel_reducer.named_parameters():
        unet_train_params_list.append(para)
        para.requires_grad = True
    for name, para in pose_encoder.named_parameters():
        unet_train_params_list.append(para)
        para.requires_grad = True
    for name, para in linear_fuse.named_parameters():
        unet_train_params_list.append(para)
        para.requires_grad = True
    ...
    if args.mixed_precision == "fp16":
        ...
        cast_training_params(channel_reducer, dtype=torch.float32)
        cast_training_params(pose_encoder, dtype=torch.float32)
        cast_training_params(linear_fuse, dtype=torch.float32)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
            ...
            if accelerator.is_main_process:
                for i, model in enumerate(models):
                    if hasattr(model, 'save_pretrained'):
                        ...
                        # Fallback to saving model state dict for models without 'save_pretrained'
                        torch.save(model.state_dict(), os.path.join(output_dir, f"model_{i}_state_dict.pth"))
        ...
        def load_model_hook(models, input_dir):
            for _ in range(len(models)):
                ...
                # Check if the model was saved with 'save_pretrained'
                model_path = os.path.join(input_dir, "unet")
                if os.path.exists(model_path):
                    ...
                    # Load the state dict for models without 'save_pretrained'
                    state_dict_path = os.path.join(input_dir, f"model_{len(models)}_state_dict.pth")
    ...
    train_dataset = StableVideoDataset(video_data_dir=args.train_data_dir, 
                                       annotations_dir=args.train_annotations_dir,
    ...
    def collate_fn(examples):
        ...
        plucker_embedding = torch.stack([example["plucker_embedding"] for example in examples])
        plucker_embedding = plucker_embedding.to(memory_format=torch.contiguous_format).float()
        return {"pixel_values": pixel_values, "conditions": conditions, "plucker_embedding": plucker_embedding,}
    ...
    unet, channel_reducer, pose_encoder,linear_fuse,optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        unet,channel_reducer,pose_encoder,linear_fuse,optimizer, train_dataloader, lr_scheduler
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                batch["plucker_embedding"] = batch["plucker_embedding"].to(accelerator.device).to(dtype=weight_dtype).permute(0, 2, 1, 3, 4)
                # Assume batch["plucker_embedding"] has shape [batch_size, num_frames, 6, H, W]
                assert batch["plucker_embedding"].ndim == 5
                bs = batch["plucker_embedding"].shape[0]

                # Process plucker_embedding through pose_encoder
                # The pose_encoder returns a list of tensors (features at different levels)
                pose_embedding_features = pose_encoder(batch["plucker_embedding"])  # Output: list of tensors
                ...
                conditions_latent += pose_embedding_features
                # Permute to bring channel dimension (4) last for the linear layer
                conditions_latent = conditions_latent.permute(0, 1, 3, 4, 2).contiguous()  # Shape: [batch, depth, height, width, channels]
                
                # Apply Linear layer across the channel dimension (now last)
                conditions_latent = linear_fuse(conditions_latent)
                
                # Permute back to original shape [batch, channels, depth, height, width]
                conditions_latent = conditions_latent.permute(0, 1, 4, 2, 3).contiguous()
                ...
                model_pred = unet(noisy_latents_inp, timesteps,
                                ...
                                # added_cond_kwargs={'pose_features': pose_features_flat},
