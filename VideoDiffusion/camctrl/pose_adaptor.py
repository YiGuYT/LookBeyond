...
from camctrl.motion_module import TemporalTransformerBlock
...
class CameraPoseEncoder(nn.Module):
    def __init__(self,
        ...
        # Construct only layers required up to output[-4]
        for i in range(len(channels[:len(channels)-3])):  # Adjust to limit to output[-4]
    ...
    def forward(self, x):
        ...
        # extract features up to output[-4]
        ...
        return x  # Return the final processed feature at output[-4]
